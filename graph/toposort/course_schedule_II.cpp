...
vector<int> possibleOrdering(int numCourses, vector<pair<int, int>> &prerequisites){
    ...
    if(linearordering.size() == numCourses) return linearordering; //means we found DAG hence a valid toposorting had been done on it 
    else return {}; // cycle exist in graph 
}
...
int main(){
    ...
    cout << "Possible linear ordering : ";
    for(auto ele : possibleOrdering(numofcourses, dpd)){
        cout << ele << " ";
    }
    cout << endl;
    ...
    numofcourses = 2;
    cout << "Possible linear ordering : ";
    for(auto ele : possibleOrdering(numofcourses, dpd2)){
        cout << ele << " ";
